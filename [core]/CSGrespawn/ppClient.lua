...
turfCancelTimer=false
addEventHandler("onClientGUIClick",root,function()
	if source == btnHos then
		turfCancelTimer = setTimer(function() triggerServerEvent("cancelCustomSpawn",localPlayer) end,1000,0)
		guiSetVisible(window,false)
		showCursor(false)
	end
	if source == btnPlace then
		...
		if teamName == "SWAT" then
			triggerServerEvent("spawnLawPlayer",localPlayer,1275.77, -1634.05, 14.58,88,"You have been taken to SWAT Base")
		elseif teamName=="Government Agency" then
			triggerServerEvent("spawnLawPlayer",localPlayer, 2011.33, -160.7, 35.58 ,357,"You have been taken to FBI Base")
...
function saveSpawn ( )
  startSaveSpawn()
  if isTimer(turfCancelTimer) then killTimer(turfCancelTimer) end
  turfCancelTimer=false
end
