...
local signUpBlip
local signUpMarker
local stopRaceToSignUpDelay = 900000 -- 15 minutes
...
function raceTime(pSource)
	...
	if isTimer ( signUpTimer ) then 
		...
		exports.DENdxmsg:createNewDxMessage ( pSource, tostring( timeLeft ).." left before sign-up starts.", 0, 255, 0 )
		
	elseif isTimer ( signUpEndTimer ) then 
	
		local timeLeft, executeLeft, executeTotal = getTimerDetails ( signUpEndTimer ) 
		local timeLeft = formatTime(timeLeft)
		exports.DENdxmsg:createNewDxMessage ( pSource, tostring ( timeLeft ).." left before sign-up ends.", 0, 255, 0 )
		
	elseif racers then
	
		exports.DENdxmsg:createNewDxMessage ( pSource, "There is a race going on right now!", 0, 255, 0 )
	...
	if fileList then
		...
		if signUpTrack and fileExists( "tracks\\"..signUpTrack..".track") then
			...
			for i=1, #getElementsByType ( "player" ) do
				...
				exports.DENdxmsg:createNewDxMessage ( getElementsByType ( "player" )[i], announceMessage, 0, 255, 0 )
...
copTeams = { ["Police"]=true, ["Military Forces"]=true, ["SWAT"]=true }
	...
	if #signedUpPlayers >= 3 then
		...
		for i=1, #signedUpPlayers do
			...
			if isElement ( signedUpPlayers[i] ) then
				...
				exports.DENdxmsg:createNewDxMessage ( signedUpPlayers[i], "There were not enough players to start the race!", 255, 0, 0 )
...
function onNFSPlayerFinish ()
	...
	-- manage rewards
	exports.CSGaccounts:addPlayerMoney ( source, firstReward )
...
function notifyRacersOf(message)
	...
	for i=1, #racers do
		...
		if isElement(racers[i]) then
			...
			exports.DENdxmsg:createNewDxMessage ( racers[i], message, 0, 255, 0 )
