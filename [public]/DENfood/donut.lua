...
function donutHide()
	if guiGetVisible(donutWindow) then
		guiSetVisible(donutWindow, false) showCursor(false,false)
	end
end


function buyDonut ()
 if getElementDimension(localPlayer) == 0 and not isPedInVehicle(localPlayer) then guiSetVisible(donutWindow, false) return end
