...
txtamm = guiCreateEdit(161, 169, 129, 28, "money am", false, GUIEditor.tab[2])
...
txtamd = guiCreateEdit(200, 100, 53, 31, "drug am", false, GUIEditor.tab[3])
...
txtamw = guiCreateEdit(200, 100, 53, 31, "weps am", false, GUIEditor.tab[4])
...
lblplayingsong = guiCreateLabel(111, 8, 200, 15, "playing this song", false, GUIEditor.tab[5])
...
function updateData()
	...
	if hasAllAccess == false then
		...
		if cancontrolmusic == true then
			guiSetEnabled(btnaddsong,true) guiSetEnabled(btnplay,true) guiSetEnabled(btnstop,true) guiSetEnabled(btndeletesong,true)
			else
			guiSetEnabled(btnaddsong,false) guiSetEnabled(btnplay,false) guiSetEnabled(btnstop,false) guiSetEnabled(btndeletesong,false)
		end
	end
	--money
...
function updateMyDrugs()
	...
	if drugsTable == nil then return end
...
function click()
	...
	elseif source == btndw then
		...
		if row ~= -1 and row ~= false and row ~= nil then
			...
			id=tonumber(id)
			local houseWeapons = fromJSON(ged("weaponsStorage"))
			if houseWeapons == nil then houseWeapons = {} end
			if houseWeapons[id] == nil then houseWeapons[id] = 0 end
			for k,v in pairs(myWeps) do
				if getWeaponIDFromName(v[1]) == id then
					if tonumber(v[2]) < text then
			...
			houseWeapons[id] = houseWeapons[id]+text
			triggerServerEvent("CSGhousing.weaponsTrans",localPlayer,id,text,"","","Deposit",toJSON(houseWeapons),houseEDATA)
		else
	...
	elseif source == btnww then
		...
		if row ~= -1 and row ~= false and row ~= nil then
			...
			triggerServerEvent("CSGhousing.weaponsTrans",localPlayer,id,text,totalAmmoInSlot,currID,"Withdraw",toJSON(houseWeapons),houseEDATA)
		else
			exports.dendxmsg:createNewDxMessage("You didn't select a weapon",255,255,0)
			return
		end
	...
	elseif source == btndd then
		...
		if row ~= -1 and row ~= false and row ~= nil then
			...
			local myAm = tonumber(guiGridListGetItemText(gridMyDrugs,row,2))
			if myAm < text then
