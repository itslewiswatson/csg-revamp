...
addEventHandler("CSGhousing.showHouseSettings",root,function(houseE)
		...
		local boughtprice = getElementData(houseE, "9")
		local passwordlocked = getElementData(houseE, "11")
		triggerClientEvent ( source, "showHousingGui", source, playerID, houseid, ownername, ownersid, interiorid, housesale, houseprice, housename, houselocked, boughtprice, passwordlocked, true )
end)
...
addEventHandler("onPlayerLogin",root,function()
	local dim,int = getElementDimension(source),getElementInterior(int)
	int=tonumber(int)
	if allHouses[int] == nil then return end
	if allHouses[int][dim] == nil then return end
	local theHouse = allHouses[int][dim]
