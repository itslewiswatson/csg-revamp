	...
	function ()
		...
		addIRCCommandHandler("!mutes",
			function (server,channel,user,command)
				...
				if type(results) == "table" then
					if #results == 0 then
						ircSay(channel,"12* There are no muted players")
					else
						for i,result in ipairs (results) do
							ircSay(channel,"12* "..tostring(result.player).." by "..tostring(result.admin).." for: "..tostring(result.reason).." during: "..tostring(getTimeString(result.duration)))
						end
					end
				else
					ircSay(channel,"12* No mutes")
				end
			end
		)
		...
		addIRCCommandHandler("!wantedlevel",
			function (server,channel,user,command,name)
				...
				local player = getPlayerFromPartialName(name)
				if player then
					ircSay(channel,getPlayerName(player).."'s wanted level: "..tostring(getPlayerWantedLevel(player)))
				else
		...
		addIRCCommandHandler("!team",
			function (server,channel,user,command,name)
				...
				if player then
					...
					if team then
						ircSay(channel,getPlayerName(player).."'s team: "..getTeamName(team))
					else
						ircSay(channel,getPlayerName(player).." is in no team")
					end
				else
					ircSay(channel,"'"..name.."' no such player")
				end
		...
		addIRCCommandHandler("!ping",
			function (server,channel,user,command,name)
				...
				local player = getPlayerFromPartialName(name)
				if player then
					ircSay(channel,getPlayerName(player).."'s ping: "..getPlayerPing(player))
				else
					ircSay(channel,"'"..name.."' no such player")
				end
			end
