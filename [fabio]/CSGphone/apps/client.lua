...
function onClientChatSystemMessage ()
	...
	if not ( theMessage ) or ( theMessage:match("^%s*$") ) or ( theMessage == "" ) or ( theMessage == " " ) then
		...
		if theTabName == "Alliancechat" and getElementData(localPlayer,"alliance") == false then
			exports.DEndxmsg:createNewDxMessage("Your not in a alliance",255,0,0)
			return
		end
...
CSGChatWindow = guiCreateWindow(370,228,705,447,"CSG ~ Chatsystem",false)
CSGChatTabpanel = guiCreateTabPanel(9,25,687,411,false,CSGChatWindow)
...
for i, roomName in ipairs ( chatRooms ) do
	...
	chatSystemGUI[roomName][2] = guiCreateGridList( 2, 4, 683, 356, false, chatSystemGUI[roomName][1] )
	chatSystemGUI[roomName][3] = guiCreateEdit( 1, 359, 564, 27, "", false, chatSystemGUI[roomName][1] )
	chatSystemGUI[roomName][4] = guiCreateCheckBox( 569, 362, 112, 21, "Show in chatbox", false, false, chatSystemGUI[roomName][1] )
	...
	if ( roomName == "NL" ) or ( roomName == "TN" ) or ( roomName == "TR" ) or ( roomName == "RU" ) or ( roomName == "AR" ) or ( roomName == "Cafe" ) or (roomName=="PT/BR") or (roomName=="PH") or (roomName=="EG") then
		...
		exports.DENsettings:setPlayerSetting( "chatOutput"..roomName, "true")
	end
