local chatRooms = {
"Support", "Mainchat", "Localchat", "Teamchat", "Groupchat","Alliancechat", "Cafe", "NL", "TN", "TR", "RU", "AR","PT/BR","PH","EG"
}
...
addCommandHandler("chatsyslog",function()
	logData=true
end)

addCommandHandler("chatsysunlog",function()
	logData=false
end)
	...
	if ( exports.server:getPlayerAccountName ( source ) ) then
		if ( messageType == 1 ) then
			...
			elseif ( actionMessageSpam[source] ) and ( getTickCount()-actionMessageSpam[source] < 2000 ) then
				exports.DENdxmsg:createNewDxMessage(source, "You are typing to fast! The limit is one message per 2 seconds.", 200, 0, 0)
			else
...
function onPlayerMessageTeamChat( message, messageType, thePlayer )
...
function onPlayerMessageMainChat( message, messageType, thePlayers )
	...
	if ( exports.server:getPlayerAccountName ( source ) ) then
		...
		elseif ( messageType == 0 ) and ( exports.CSGadmin:getPlayerMute ( source ) == "Main" ) or ( exports.CSGadmin:getPlayerMute ( source ) == "Global" ) then
			exports.DENdxmsg:createNewDxMessage(source, "You are muted!", 236, 201, 0)
			cancelEvent()
		elseif ( messageType == 0 ) and ( mainChatSpam[source] ) and ( getTickCount()-mainChatSpam[source] < 6000 ) and not ( exports.CSGstaff:isPlayerStaff( source ) ) then
			exports.DENdxmsg:createNewDxMessage(source, "You are typing to fast! The limit is one message 6 seconds.", 200, 0, 0)
			cancelEvent()
		elseif messageType == 0 then
			...
			for theKey, thePlayer in ipairs(getAllPlayers) do
				...
				if thePlayerChatZone == playerChatZone then
					table.insert(nearbyPlayers,thePlayer)
				end
			end

			for index, nearbyPlayer in ipairs( nearbyPlayers ) do
				if ( getElementData( nearbyPlayer, "chatOutputMainchat" ) ) then
					outputChatBox( "(".. playerChatZone ..") ".. getPlayerName(source) ..": #FFFFFF".. removeHEX(message), nearbyPlayer, r,g,b, true )
				end
			end
		...
		if ( theMessage ) and ( theRoom ) then
			...
			elseif ( theRoom == "Teamchat" ) then
				...
				elseif ( chatRoomSpam[source] ) and ( getTickCount()-chatRoomSpam[source] < 1000 ) then
					...
					local ally = getElementData(source,"alliance")
					for k, thePlayer in ipairs ( getElementsByType( "player" ) ) do
						...
						if ( getElementData( thePlayer, "chatOutput"..theRoom ) ) then
							...
							elseif theRoom == "alliance" then
								if getElementData(thePlayer,"alliance") == ally then
									outputChatBox("(" .. string.upper(theRoom) .. ") " .. ( getPlayerName( source ) ) .. ": #FFFFFF"..( theMessage ).." ", thePlayer, 0,255, 0, true)
...
function onChatSystemCommandChat( thePlayer,commandName,... )
	if ( commandName == "nl" ) or ( commandName == "tn" ) or ( commandName == "tr" ) or ( commandName == "ru" ) or ( commandName == "ar" ) or ( commandName == "pt" )  or ( commandName == "br" ) or (commandName == "ph") or (commandName == "eg") then
...
for i=1,#chatRooms do
	if ( chatRooms[i] == "NL" ) or ( chatRooms[i] == "TN" ) or ( chatRooms[i] == "TR" ) or ( chatRooms[i] == "RU" ) or ( chatRooms[i] == "PT/BR" ) or ( chatRooms[i] == "AR" ) or ( chatRooms[i] == "Support" ) or ( chatRooms[i] == "Cafe") or (chatRooms[i] == "PH") or (chatRooms[i] == "EG") then
