...
addEventHandler ( "onResourceStart", getResourceRootElement( getThisResource () ),
	function ()
		connection = dbConnect( "mysql", "dbname=mtasa;host=127.0.0.1;port=3306;unix_socket=/var/run/mysqld/mysqld.sock", "mtasa", exports.DENmysql:getPass() )
		if ( connection ) then
...
function save(p)
	...
	if querys[p] ~= nil then
		dbFree(querys[p])
	end
	querys[p]=dbQuery(saveCallBack1,{accName,userid,str},connection, "SELECT timestamp FROM crimes WHERE userid=? AND timestamp=?",userid,str)
end

function saveCallBack1(qh,accName,userid,str)
	...
	querys[p]=dbQuery(saveCallBack2,{accName,userid},connection, "SELECT timestamp FROM crimes WHERE userid=? AND timestamp=?",userid,11111111)
end

function saveCallBack2(qh,accName,userid)
	local t2 = dbPoll(qh,0)
	if #t2 == 0 then
...
function setData(p)
	...
	querys[p]=dbQuery(setDataCallBack1,{accName,p},connection, "SELECT * FROM crimes WHERE userid=? ORDER BY timestamp DESC LIMIT 7",userid)
end

function setDataCallBack1(qh,accName,p)
	local t = dbPoll(qh,0)
 	if #t ~= 0 then
	...
	querys[p]=dbQuery(setDataCallBack2,{accName,p},connection,"SELECT * FROM crimes WHERE userid=? AND timestamp=?",userid,11111111)
end

function setDataCallBack2(qh,accName,p)
	local t2 = dbPoll(qh,0)
	if #t2 ~= 0 then
...
function sendData(p,bool)
	...
	if accountCurrentJailPeriod[acc] == nil then setData(p) end
...
function detected(p,i,victim,cx,cy,cz)
        ...
        if isElementWithinColShape(p,LVcol) == true then
                if isLaw(p) then
