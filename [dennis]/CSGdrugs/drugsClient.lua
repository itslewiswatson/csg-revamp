...
function windowToggle()
	if (getElementHealth(localPlayer) ~= 0) then
		if guiGetVisible(DrugsGUI[1]) == true then
			...
			if getElementData(localPlayer,"ho") == true then
				exports.dendxmsg:createNewDxMessage("Can't open drugs panel while house panel is open!",255,0,0)
				return
			end
			updateLabels()
...
if not (tonumber(amount) >= 1) then
	...
	if tonumber(amount) ~= math.floor(tonumber(amount)) then
		...
		exports.dendxmsg:createNewDxMessage("Drug taking amounts can only be whole numbers only! eg. 1, 2, 3 not 0.01, 0.02, etc!",255,0,0)
		return
	end
