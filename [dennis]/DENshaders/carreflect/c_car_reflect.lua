...
function EnableCarReflect()
	...
	if not Variables.enabled then
		...
		-- Create shader
		if not ( grunShader and geneShader and shatShader ) then
			grunShader = dxCreateShader ( "carreflect/car_refgrun.fx",1,Variables.renderDistance,false)
			geneShader = dxCreateShader ( "carreflect/car_refgene.fx",1,Variables.renderDistance,true)
			shatShader = dxCreateShader ( "carreflect/car_refgene.fx",1,Variables.renderDistance,true)
		end
		...
		if not grunShader or not geneShader or not shatShader then
			...
			-- Set textures
			local textureVol = dxCreateTexture ( "carreflect/images/smallnoise3d.dds" );
...
function DisableCarReflect()
	if Variables.enabled then
		...
		Variables.enabled = false
		for _,addList in ipairs(texturegrun) do
			engineRemoveShaderFromWorldTexture (grunShader, addList )
		end

		engineRemoveShaderFromWorldTexture ( grunShader, "vehiclegrunge256" )
		engineRemoveShaderFromWorldTexture ( grunShader, "?emap*" )
		engineRemoveShaderFromWorldTexture ( geneShader, "vehiclegeneric256" )
		engineRemoveShaderFromWorldTexture ( shatShader, "vehicleshatter128" )
		
		engineRemoveShaderFromWorldTexture ( geneShader, "hotdog92glass128" )
				
		engineRemoveShaderFromWorldTexture ( geneShader, "okoshko" )
	end
end
