...
function onBankMarkerHit ()
	...
	if not balanceCheck then
		exports.DENdxmsg:createNewDxMessage(source, "Seems you didn't had a back account, we created one for you!", 200,0,0)
...
function withdrawMoney (value)
	...
	if tonumber(value) > tonumber(balanceCheck.balance) or balanceCheck.balance == 0 then
		exports.DENdxmsg:createNewDxMessage(source, "You dont have enough money on your bank account!", 200,0,0)
	else
...
function requestTransactions ()
	...
	if not transactionsCheck then
		exports.DENdxmsg:createNewDxMessage(source, "You dont have recent transactions.", 0, 200, 0)
