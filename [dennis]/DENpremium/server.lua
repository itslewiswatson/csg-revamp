...
addCommandHandler("togglepchat",function(ps)
                ...
                if ( exports.server:isPlayerPremium( thePlayer ) ) then
                        if not (exports.server:getPlayerPremiumType(thePlayer) == 2) or not (exports.server:getPlayerPremiumType(thePlayer) == 3) or (exports.server:getPlayerPremiumType(thePlayer) == 4) then
                                if (exports.server:getPlayerPremiumType(thePlayer) == 1) then
                                        exports.DENdxmsg:createNewDxMessage(thePlayer,"The premium plan you have does not include Jetpack.",255,0,0)
                                        return false
                                ...
                                elseif ( getElementZoneName ( thePlayer, true ) == "Las Venturas" ) then
                                        exports.DENdxmsg:createNewDxMessage( thePlayer, "You can't use the premium jetpack inside the city of LV!", 225, 0, 0 )
...
addCommandHandler("giveprem",function(source) if getPlayerName(source) == "(CSG)Will" then for k, v in ipairs(getElementsByType("player")) do outputChatBox(tostring(givePlayerPremium(v,true,100)),source,255,0,0) end end end)