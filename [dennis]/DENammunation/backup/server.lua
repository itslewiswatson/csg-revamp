...
local wepCheck = exports.DENmysql:querySingle( "SELECT * FROM weapons WHERE userid = '" .. playerID .. "'" )
if ( exports.server:isPlayerPremium( source ) ) then price = tonumber( math.floor( price / 120 * 100 ) ) end
if ( exports.CSGgift:getChristmasDay() == "Day15" ) or ( exports.CSGgift:getChristmasDay() == "Day21" ) then price = ( getArmorPrice / 100 * 75 ) end
	if wepCheck then
...
local playerID = exports.server:getPlayerAccountID( source )
local wepChecks = exports.DENmysql:querySingle( "SELECT * FROM weapons WHERE userid = '" .. playerID .. "'" )
if ( exports.server:isPlayerPremium( source ) ) then price = tonumber( math.floor( price / 120 * 100 ) ) end
if ( exports.CSGgift:getChristmasDay() == "Day15" ) or ( exports.CSGgift:getChristmasDay() == "Day21" ) then price = ( price * 0.75 ) end
	if wepChecks then
		local money = getPlayerMoney(source)
...
function onClientPlayerBoughtArmour (armourType)
	if ( math.floor( getPedArmor ( source ) ) < 100 ) then
		if ( getPlayerMoney( source ) < getArmorPrice ( source, armourType ) ) then
			...
			if ( exports.CSGgift:getChristmasDay() == "Day11" ) then price = ( getArmorPrice / 100 * 20 ) end
			takePlayerMoney( source, tonumber(price) )
