...
addEventHandler("ReceiveServerData",root,function(weight_limit,hang_binds_,hang_commands_,drop_binds_,drop_commands_)
	SetHangingWeightLimit(weight_limit)
	
	hang_binds = hang_binds_
	hang_commands = hang_commands_
	drop_binds = drop_binds_
	drop_commands = drop_commands_
	
	if hang_commands then
		for _,command in ipairs(hang_commands) do
			addCommandHandler(command,AttemptHeliGrab)
		--	outputChatBox("Adding command "..tostring(command).." for grab")
		end	
	end	
	
	if hang_binds then
		for _,bind in ipairs(hang_binds) do
			bindKey(bind,"down",AttemptHeliGrab)
		--	outputChatBox("Binding "..tostring(bind).." to grab")
		end
	end
	
	if drop_commands then
		for _,command in ipairs(drop_commands) do
			addCommandHandler(command,AttemptHeliDrop)
		--	outputChatBox("Adding command "..tostring(command).." for drop")
		end	
	end	
	
	if drop_binds then
		for _,bind in ipairs(drop_binds) do
			bindKey(bind,"down",AttemptHeliDrop)
		--	outputChatBox("Binding "..tostring(bind).." to drop")
		end
	end
end)
...
addEventHandler("onClientResourceStart",getResourceRootElement(getThisResource()),function()
	-- ask the server for the current hanging_weight_limit value and the binds/commands
	triggerServerEvent("RequestServerData",local_player)
...
function AttemptHeliDrop()
	triggerEvent("PlayerDrop",local_player,"manual",target.vehicle)
end



function GrabHeli(heli,side,line_percent)
	local data = getElementData(source,"hanging")
	-- already hanging and changing to new position, so drop and immediately re-attach
	if data and ((side ~= data.side) or (line_percent ~= data.line_percent) or (heli ~= data.heli)) then	
		triggerEvent("PlayerDrop",source,"changing hang",data.heli)
	end	
...
function UpdateHangingEffect()
	if getElementData(local_player,"hanging") and target.vehicle then		
...
addEventHandler("onClientRender",root,function()
			...
			if heli and isElement(heli) then
				if isElementOnScreen(heli) then
					...
					if getDistanceBetweenPoints3D(hx,hy,hz,left_hand_x,left_hand_y,left_hand_z) < track_distance then
						...
						local offset = {}
						if helicopter_offsets[veh_name] then
	...
	for _,p in ipairs(getElementsByType("player", root, true)) do
		if isElementStreamedIn(p) then
			...
			if player_hanging and player_hanging.heli then			
				-- attach to heli
		...
		if vehicle == v then
			...
			if vehicle == target.vehicle then
				if getElementData(local_player,"hanging") then
					triggerEvent("PlayerDrop",local_player,"heli_destroyed",target.vehicle)
				end
				target.vehicle = nil
				target.distance = track_distance
			end
