function updateWantedLevel(player)
	setPlayerNametagText(player,getPlayerName(player).." ("..tostring(getPlayerWantedLevel(player))..")")
end
addEventHandler("onPlayerChangeNick",root,function() updateWantedLevel(source) end)
...
function onPlayerNightstickArrest ( attacker, weapon, bodypart, loss )
	if isElement(attacker) and (weapon == 3) and ( getElementType ( attacker ) == "player" ) then
...
function onWantedPlayerGotTazerd ( prisoner, dogTaz )
	...
	if (timeToHit[prisoner]) then
		...
		exports.dendxmsg:createNewDxMessage(source,"Tazer nearly missed the target - re-hit within 10s",255,255,0)
		return
		end
	end
	...
	exports.dendxmsg:createNewDxMessage(source,"Tazer Successfull",0,255,0)
	...
	if ( exports.server:getPlayerAccountName ( thePlayer ) ) then
		...
		for i, theCop in ipairs ( getElementsByType( "player" ) ) do
			...
			if ( exports.server:getPlayerAccountName ( theCop ) ) then
				if ( getPlayerTeam(theCop) ) and ( (getTeamName(getPlayerTeam(theCop)) == "Police") or (getTeamName(getPlayerTeam(theCop)) == "SWAT") or (getTeamName(getPlayerTeam(theCop)) == "Military Forces") or (getTeamName(getPlayerTeam(theCop)) == "Government Agency") ) then
...
function onDataChange(name)
	if name == "isPlayerArrested" and getElementType(source) == "player" then
