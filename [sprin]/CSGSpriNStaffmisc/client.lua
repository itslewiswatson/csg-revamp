...
bindKey("fire","down",function()
	...
	local ggun_obj = getElementData(localPlayer,"ggun_takenn")
	...
	elseif getKeyState(push_button) then
		...
		triggerServerEvent("ggun_pushh",ggun_obj,x2,y2,z2)
	else
		if not isElement(ggun_obj) then
			local target = getPedTarget(localPlayer)
			if target and getElementType(target) ~= "object" then
				triggerServerEvent("ggun_takee",target)
			end
		else
			triggerServerEvent("ggun_dropp",root)
		end
	end
end)

bindKey("aim_weapon","up",function()
	if isElement(getElementData(localPlayer,"ggun_takenn")) then
		triggerServerEvent("ggun_dropp",root)
	end
end)
...
addEventHandler("onClientPreRender",root,function()
	...
	for plnum,player in ipairs(all_players) do
		local taken = getElementData(player,"ggun_takenn")
