		...
		if (currentTick - scoreboardTicks.lastUpdate > scoreboardTicks.updateInterval and (scoreboardToggled or scoreboardForced)) or forceScoreboardUpdate then
			...
			for key, player in ipairs( players ) do
				if not getPlayerTeam( player ) or not (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) or serverInfo.forcehideteams then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						elseif column.name == "loc" then
							content = {}
							content.type="image"
							local con = getElementData(player,"loc") if con == false then con = "" end

							if con == "" then
								content.src=""..".png"
							else
								content.src=":CSGflags/flags/"..con..".png"
							end
							content.height=11
							content.width=16
						else
			...
			if (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) and not serverInfo.forcehideteams then
				...
				for k,v in pairs(teams) do
					if getTeamName(v) == "Criminals" then crimsk = k break end
				end
				...
				table.remove(teams,smurfsk)
				table.insert(teams,getTeamFromName("The Smurfs"))
				for k,v in pairs(teams) do
					if getTeamName(v) == "The Terrorists" then terroristsk = k break end
				end
				...
				for key, team in ipairs( teams ) do
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "name" then
							...
							if teamName == "The Smurfs" then
								local i = 0
								for k,v in pairs(getElementsByType("player")) do
									if getElementModel(v) == 261 or getElementModel(v) == 57 then i = i+1 end
								end
								teamMemberCount = i
							elseif teamName == "The Terrorists" then
								local i = 0
								for k,v in pairs(getElementsByType("player")) do
									if getElementModel(v) == 136 then i=i+1 end
								end
								teamMemberCount = i					
							end
							if teamName == "The Smurfs" or teamName == "The Terrorists" then
							--	teamName = ""..teamName .. ""
					...
					if getTeamName(team) == "The Smurfs" then
						players = getPlayersInTeam(getTeamFromName("Criminals"))
						local t = {}
						for k,v in pairs(players) do
							local gr = getElementData(v,"Group")
							if gr == "The Smurfs" then if getElementModel(v) == 261 or getElementModel(v) == 57  then table.insert(t,v) end end
						end
						players = t
					elseif getTeamName(team) == "The Terrorists" then
						players = getPlayersInTeam(getTeamFromName("Criminals"))
						local t = {}
						for k,v in pairs(players) do
							local gr = getElementData(v,"Group")
							if gr == "The Terrorists" and getElementModel(v) == 136 then table.insert(t,v) end
						end
						players = t						
					elseif getTeamName(team) == "Criminals" then
						local t = {}
						for k,v in pairs(players) do
							local pSkin = getElementModel(v)
							local gr = getElementData(v,"Group")
							if ( gr ~= "The Smurfs" or (pSkin ~= 261 and pSkin ~= 57) ) and ( gr ~= "The Terrorists" or (pSkin ~= 136) ) then
								table.insert(t,v)
							end
						end
						players=t
					...
					for key, player in ipairs( players ) do
						...
						for key, column in ipairs( scoreboardColumns ) do
							...
							elseif column.name == "Rank" then
								...
								if isLaw(player) == true then
									if showingRank == true then
										content = getElementData( player, column.name ) or ""
									else
										local skill = getElementData(player,"skill")

										if skill ~= false and skill ~= nil and skill ~= "" and skill ~= "Regular Officer" then
											content = getElementData( player, "skill" )
										else
											content = getElementData( player, column.name )
										end
									end
							...
							elseif column.name == "Group" then
								if showingRank==false then
									local temp = getElementData( player, "aName" )
									if temp ~= false then
										content = temp
									else
										content = getElementData( player, column.name )
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if content and column.name == "name" then
							if useColors then
								...
								--local gr = getElementData(element,"Group")
								--if gr == "The Smurfs" then r,g,b = 148,0,211 end
							end
