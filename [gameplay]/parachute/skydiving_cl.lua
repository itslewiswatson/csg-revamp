...
local function onRender()
	...
	if tickDiff > 0 then
		if ( getPedWeapon ( localPlayer, 11 ) == 46 ) and not getElementData(localPlayer, "parachuting") and not doesPedHaveJetPack(localPlayer) and not isElementAttached(localPlayer) and not isPlayerDead(localPlayer) and not (getElementData(localPlayer,"superman:flying") == true) then
			...
			and not isElementInWater(localPlayer) and not testLineAgainstWater ( x,y,z,x,y,z + 12) then	
				...
				if not getElementData(localPlayer,"skydiving") then
					...
					if not (warning == false) and processLineOfSight ( x,y,z, x,y,z-WARNING_HEIGHT, true, false,false,true,false,false,false,false,localPlayer ) then
...
function isPlayerSkyDiving(player)
	return getElementData(player, "skydiving")
end