...
function onUpdateHouseData ( houseID, column1, data1, column2, data2, column3, data3 )
	if ( houseID ) then
		...
		elseif ( column1 ) and ( column2 ) then
...
-- Create houses
function createHouse ( houseid, ownerid, interiorid, housex, housey, housez, housesale, houseprice, housename, houselocked, boughtprice, passwordlocked, housepassword, weaponsStorage, drugsStorage, moneyStorage, perms )
	if ( housesale == 0 ) then
	...
	if allHouses[tonumber(interiorid)] == nil then
		allHouses[tonumber(interiorid)] = {}
	end
	allHouses[tonumber(interiorid)][playerdim] = house
end
...
function ( thePlayer )
	...
	if not ( theVehicle ) then
		local houseElementID = getElementData(source, "houseid")
		...
		if ( source == houseElement ) then
			...
			local ownersid = getElementData(source, "ownerid")
			local ownername = getElementData(source, "ownername")
			local interiorid = getElementData(source, "interiorid")
			local housesale = getElementData(source, "housesale")
			local houseprice = getElementData(source, "houseprice")
			local housename = getElementData(source, "housename")
			local houselocked = getElementData(source, "houselocked")
			local boughtprice = getElementData(source, "boughtprice")
			local passwordlocked = getElementData(source, "passwordlocked")
...
function enterHouse (houseid, interiorid, isPassword, password)
	...
	local ownersid = getElementData( theHouse, "ownerid" )
	local playersid = exports.server:getPlayerAccountID( source )
	...
	if ( isPassword ) or ( sourceTeam == "Staff" ) or ( sourceTeam == "SWAT" ) or ( sourceTeam == "Military Forces" ) then
		if ( tostring(getElementData(theHouse, "housepassword")) == tostring(password) ) or ( sourceTeam == "Staff" ) or ( sourceTeam == "SWAT" ) or ( sourceTeam == "Military Forces" ) then
			local playerdim = exports.DENmysql:querySingle( "SELECT * FROM housing WHERE id = ?", houseid )
			...
			triggerClientEvent(source,"CSGhousing.enteredHouse",source,theHouse,myAccName)
		else
			outputChatBox("Wrong house password! Try again.", source, 225, 0, 0)
		end
	else
		if ( ownersid ~= playersid ) and ( getElementData(theHouse, "passwordlocked") == 1 ) then
...
function setHousePassword ( houseid, password )
	local theHouse = _G["house"..tostring(houseid)]
	setElementData( theHouse, "housepassword", password )
	setElementData( theHouse, "passwordlocked", 1 )
	setElementData( theHouse, "houselocked", 1 )
	onUpdateHouseData ( houseid, "locked", 1, "passwordlocked", 1, "password", password )
...
function removeHousePassword ( houseid )
	local theHouse = _G["house"..tostring(houseid)]
	if ( getElementData( theHouse, "passwordlocked" ) == 1 ) then
		local theHouse = _G["house"..tostring( houseid )]
		setElementData( theHouse, "passwordlocked", 0 )
		onUpdateHouseData ( houseid, "passwordlocked", 0 )
		onPlayerUpdateLabels ( source, houseid )
...
function toggleClosed (houseid, houseinglocked)
	local theHouse = _G["house"..tostring( houseid )]
	local houseLockStatus = getElementData( theHouse, "houselocked" )
...
function toggleSalePrice (houseid, thePrice)
	local theHouse = _G["house"..tostring( houseid )]
	setElementData( theHouse, "houseprice", tonumber(thePrice))
	onUpdateHouseData ( houseid, "houseprice", tonumber(thePrice) )
	onPlayerUpdateLabels ( source, houseid )
end
...
function buyHouse (houseid)
	local theHouse = _G["house"..tostring( houseid )]
	local theHousePrice = getElementData( theHouse, "houseprice" )
	local theHouseOwnerID = getElementData( theHouse, "ownerid" )
	local isHouseForSale = getElementData( theHouse, "housesale" )
	...
	elseif ( isHouseForSale == 0 ) then
		...
		if ( theHouseOwnerBalance ) then
			...
			if ( theOwnerGiveMoney ) then
				...
				setElementData( theHouse, "ownername", playerAccountName)
				setElementData( theHouse, "ownerid", playerID)
				setElementData( theHouse, "boughtprice", theHousePrice)
				onUpdateHouseData ( houseid, "ownerid", playerID, "boughtprice", theHousePrice, "sale", 0 )
				setElementData( theHouse,"housesale", 0)
				setPickupType( theHouse, 3, 1272, 0)
				triggerClientEvent ( source, "closeBuyWindow", source)
