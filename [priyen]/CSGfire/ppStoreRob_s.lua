...
local storeRobMarkers = {}
local storeMoneyBags = {}
local storeBagRotateTimers = {}
local storeBagSizeTimers = {}
...
function endRobbery(store)
	...
	if isElement(storeMoneyBags[markers[store]]) then
		destroyElement(storeMoneyBags[markers[store]])
	end
    t[store]["robInProgress"] = 0
    t[store]["theRobber"] = 0
        ...
        if isElement(v) ~= false then
            if isElementWithinMarker(thePlayer, v) then
                if getElementDimension(v) == getElementDimension(thePlayer) then
					store = markers[k]
					theMarker = v
					storeRobMarkers[store]=theMarker
                break
                end
            end
        end
            ...
            if t[markers[store]]["timeUntilRob"] == 0 then
                   local chance = math.random(0,100)
                    if chance > t[markers[store]]["chanceToRob"] then
						...
						storeMoneyBags[markers[theMarker]] = obj
						setObjectScale(obj,0.5)
						setElementInterior(obj,int)
						setElementDimension(obj,dim)
						local timer = setTimer(function() local scale = getObjectScale(obj) setObjectScale(obj,scale+0.031) end,1000,0)
						storeBagSizeTimers[store] = timer
						--attachElements(obj,thePlayer,-0.3,0.75)
                    else
                        exports.CSGwanted:addWanted(thePlayer, 24)
...
addCommandHandler("robhere", holdup2)
                ...
                if t[k]["robbedMoney"] <= t[k]["randomRob"] then
                    ...
                    triggerClientEvent(t[k]["theRobber"],"CSGstoreRobberiesShowRobberyDialog",root,t[k]["robbedMoney"])
                         if t[k]["robbedMoney"] > t[k]["randomRob"] then
							...
							if isTimer(storeBagSizeTimers[k]) then
								killTimer(storeBagSizeTimers[k])
							end
                         end
                    end
                end
   ...
   local interiorEntries = getElementsByType ( "interior", getResourceRootElement(res) )
	...
	for k,v in pairs(t) do
		...
		for k2,v2 in pairs(tableToUse) do
			local ped = createPed(1,v2[1],v2[2],v2[3],v2[4],true)
