...
addEventHandler( "onServerPlayerLogin", root,
	function ()
		dbQuery(getDataCB,{source},exports.DENmysql:getConnection(),"SELECT * FROM playerstats WHERE userid=? LIMIT 1", exports.server:getPlayerAccountID ( source ) )
	end
)
...
function setPlayerAccountData ( pE, data, value )
 	if isElement(pE) then
		...
		if (userID) then
			...
			if ( statsTable[pE] ) then statsTable[pE][data] = value changed[pE][data]=true end
			for k,v in pairs(getElementsByType("player")) do
...
for k,v in pairs(getElementsByType("player")) do
	if exports.server:isPlayerLoggedIn(v) == true then
		dbQuery(getDataCB,{v},exports.DENmysql:getConnection(),"SELECT * FROM playerstats WHERE userid=? LIMIT 1", exports.server:getPlayerAccountID ( v ) )
	end
end
end,5000,1)
--[[
...
--[[
addCommandHandler("savemystats",function(ps)
local userid = exports.server:getPlayerAccountID(ps)
	local str = ""
	local first=true
	for k,v in pairs(statsTable[ps]) do
		if first == true then
			first=false
			str = str.." `"..k.."`=`"..v.."`"
		else
	...
	exports.DENmysql:exec('UPDATE playerstats SET'..str..' WHERE userid='..userid..'')
	outputDebugString('UPDATE playerstats SET'..str..' WHERE userid='..userid..'')
end)
--]]
...
--[[local userid = exports.server:getPlayerAccountID(ps)
	...
	local loadSTR = 'exports.DENmysql:exec("UPDATE playerstats SET'..str..' WHERE userid='..userid..''
	outputDebugString(loadSTR)
	for k,v in pairs(temp) do
		loadSTR=loadSTR..',temp['..k..']'
	end
	loadSTR=loadSTR..")"
	--exports.DENmysql:exec('UPDATE playerstats SET'..str..' WHERE userid='..userid..'')
	loadstring(loadSTR)--]]
