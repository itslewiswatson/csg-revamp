...
window = guiCreateWindow(0.27, 0.17, 0.47, 0.59, "Community of Social Gaming ~ Help Menu", true)
...
GUIEditor.button[3] = guiCreateButton(9, 132, 59, 32, "", false, window)
...
"Para ver os diversos tipos de criminoso, va a marca do emprego de criminoso e proximo a ela tera outra marca,",
...
"	> Roube estabelecimentos! /holdup ou /robstore proximo a caixa registradora!",
...
"	> Procure por um circulo vermelho no mapa!",
		...
		{"Oficial de Policia",
			{
			"Aqui voce pode se juntar a policia. Oficiais da policia, em resumo, prendem bandidos. Voce pode escolher um de 8 ",
...
"Dica: para reparar um veiculo, aperte o botao direito do mouse proximo a porta. Use num_2 e num_8 para controlar o guindaste.",
...
"Paramedicos tem acesso as ambulâncias. Eles sao essenciais na vida das pessoas de San Andreas! Use seu painel para ver quem precisa ",
"dos seus servicos.",
...
"Las Venturas). Primeiro, voce deve ir ate a marca com o skin de piloto e entao obter o emprego. Depois, voce precisa obter uma licenca ",
...
"as emergencias de incendio e apaga-los usando seu extintor de incendios ou a mangueira do caminhao.",
...
"	> Obtenha mais pontos matando policiais.",
		...
		{"Roubo no Cassino :: Presente para todos os jogadores",
			{
				"Criminosos podem roubar o Cassino de Las Venturas em intervalos especificos. Fazendo isso, ganhara 6 niveis de",
	"procurado, mas a recompensa valera a pena.",
	"Voce ganhara uma boa quantia em dinheiro roubando o Cassino. Se o roubo for um sucesso, voce deve fugir e se",
	"afastar do Cassino (voce nao pode morrer enquanto nao atingir uma distância adequada).",
	"Policiais ganham 2K matando criminosos e criminosos ganham 4K matando policiais.",
	"Voce vera uma pequena tabela mostrando os 5 melhores criminosos/policiais do roubo.",
	"	> Use /casinotime para saber quanto tempo resta até o próximo roubo.",
	"	> Obtenha pontos por cada morte.",
				--"	> [Click Here to see Location]"
			}
		},
		{"Caminhao Blindado :: Presente para todos os jogadores",
			{
			"Um caminhao blindado ira surgir no mapa com a funcao de transportar dinheiro. Policiais de toda San Andreas ",
	"irao ate o caminhao para protege-lo. Apenas SWAT's e MF's poderao dirigir o caminhao e, quando comecar, o ",
	"caminhao precisa ser entregue no seu destino. Se o caminhao for entregue, os policiais irao ganhar uma boa ",
	"quantia em dinheiro. Criminosos tambem podem tentar impedir a entrega destruindo o caminhao e matando todos os ",
	"policiais envolvidos na operacao. Quando os policiais forem mortos, mochilas de dinheiro poderao ser pegas.",
	"Para ver onde o caminhao blindado esta, basta procurar pelo icone de um caminhao no mapa.",
	"	> Oficiais serao avisados quando o caminhao estiver disponivel para entrega.",
	"	> Oficiais se referem a qualquer oficial da lei.",
				--"	> [Click Here to see Location]"
			}
		},
		--[[{"Bank Robbery :: Targeted for All Players",
...
"de tempo. Se voce falhar em uma denuncia, voce falhara em toda a missao, entao dedique-se.",
		...
		{"Roubo de Lojas :: Disponivel para todos os jogadores",
			{
				"Roube uma loja se voce tiver coragem! Va ate a caixa registradora e digite /robstore ou /holdup para tentar um ",
		...
		{"Lotto 720",
			{
				...
				"Todos os dias as 18:00, a loteria sorteia seu premio e voce tem a chance de ganhar uma boa quantia em dinheiro.",
...
function setLangs(t)
	for k,v in pairs(t) do
		if k == 1 then gridToUse = gridRules end
		if k == 2 then gridToUse = gridCommands end
		if k == 3 then gridToUse = gridJobs end
		if k == 4 then gridToUse = gridEvents end
		if k == 5 then gridToUse = gridMissions end
		if k == 6 then gridToUse = gridSystems end
		if k == 7 then gridToUse = gridiPhone end
