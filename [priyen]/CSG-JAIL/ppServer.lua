...
function isPlayerInJail(p)
	return isElementWithinColShape(p,jailPremCol)
end
...
addEventHandler("onMarkerHit",enterMarker,function(p)
	if source == enterMarker then
		if isPlayerJailed(p) == true then
			setElementPosition(p,910.82, -2387.04, 5700.42)
			--cancelEvent(true)
		end
	end
end)
...
aeh("onPlayerWeaponSwitch",root,function()
	if isPlayerInJail(source)==true then
...
function getCell(p)
	for k,v in pairs(cellInmate) do
		if v == p then return k end
	end
	return false
end
aeh("onPlayerWasted",root,function()
	if isPlayerJailed(source)==true then
		local k = getCell(source)
		local x,y,z = getElementPosition(spaceMarkers[k])
		local rx=rotsForCells[k]
		msg="You have been returned to CSG's Federal Prison"
		triggerEvent("spawnLawPlayer",source,x,y,z,rx,msg)
	end
end)
