...
addEventHandler("onClientRender", root,
    function()
		if getElementData(localPlayer, "l.on") then
			if percent > 2 then
...
addEventHandler("onClientElementDataChange", localPlayer,
	function(dataName, oldValue)
		if getElementType(source) == "player" and source == localPlayer and dataName == "l.on" then
...
function AimKeyPressed(key, state) -- attempt to sync when aiming with binds, getPedControlState seems weird...
	if state == "down" then
		setElementData(localPlayer, "l.aim", true, true)
	elseif state == "up" then
		setElementData(localPlayer, "l.aim", false, true)
	end
	--outputChatBox(key .. " " .. state)
end

function DrawLaser(player)
	if getElementData(player, "l.on") then
		...
		if getElementData(player, "l.aim") and IsPlayerWeaponValidForLaser(player) == true and targetself == false then
...
addEventHandler("CSGlaser.recData",localPlayer,function(t)
	...
	setElementData(localPlayer,"l.red",r,true)
	setElementData(localPlayer,"l.green",g,true)
	setElementData(localPlayer,"l.blue",b,true)
end)

SetLaserEnabled(localPlayer,false)

setTimer(function()
	if percent < 2 then SetLaserEnabled(localPlayer,false) end
end,1000,10)
